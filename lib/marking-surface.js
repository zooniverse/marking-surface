...
(function() {
  var BACKSPACE, BaseClass, CASE_SENSITIVE_ATTRIBUTES, DELETE, ElementBase, IE_HACK_ID, Mark, MarkingSurface, NAMESPACES, NON_ATTRIBUTE_PROPERTIES, POINTER_EVENTS, PREFIXED_GRAB, PREFIXED_GRABBING, SHIFT, SVG, TAB, Tool, ToolControls, defs, elements, filters, insertStyle, name, shiftIsDown, _ref, _ref1,
  ...
  ElementBase = (function(_super) {
    ...
    ElementBase.prototype.pointerOffset = function(e) {
      var left, originalEvent, top, x, y, _ref;
  ...
  ToolControls = (function(_super) {
    ...
    ToolControls.prototype.toFront = function() {
      return this.tool.surface.toolControlsContainer.appendChild(this.el);
    };
  ...
  Tool = (function(_super) {
    ...
    Tool.prototype.select = function() {
      ...
      if (this.surface.tools[this.surface.selection] === this) {
        return;
      }
  ...
  addEventListener('keydown', (function(_arg) {
    var which;
    which = _arg.which;
    if (which === SHIFT) {
      return shiftIsDown = true;
    }
  }), false);

  addEventListener('keyup', (function(_arg) {
    var which;
    which = _arg.which;
    if (which === SHIFT) {
      return shiftIsDown = false;
    }
  }), false);
  ...
  MarkingSurface = (function(_super) {
    ...
    MarkingSurface.prototype.selection = -1;
    ...
    MarkingSurface.prototype.onStart = function(e) {
      ...
      e.preventDefault();
      tool = (this.tools[this.selection] == null) || ((_ref4 = this.tools[this.selection]) != null ? _ref4.isComplete() : void 0) ? this.createTool() : this.tools[this.selection];
      tool.select();
    ...
    MarkingSurface.prototype.onDrag = function(e) {
      var _ref2;
      e.preventDefault();
      (_ref2 = this.tools[this.selection]).onInitialDrag.apply(_ref2, arguments);
      this.triggerEvent('tool-initial-drag', this.tools[this.selection]);
      return null;
    };
    ...
    MarkingSurface.prototype.onRelease = function(e) {
      var dragEvent, _ref2;
      e.preventDefault();
      (_ref2 = this.tools[this.selection]).onInitialRelease.apply(_ref2, arguments);
      this.triggerEvent('tool-initial-release', this.tools[this.selection]);
    ...
    MarkingSurface.prototype.onFocus = function(e) {
      var _this = this;
      return setTimeout(function() {
        var index, _ref2;
        if (_this.tools[_this.selection] == null) {
          index = shiftIsDown ? _this.tools.length - 1 : 0;
          return (_ref2 = _this.tools[index]) != null ? _ref2.select() : void 0;
        }
      });
    };
    ...
    MarkingSurface.prototype.onKeyDown = function(e) {
      ...
      switch (e.which) {
        ...
        case DELETE:
          e.preventDefault();
          if ((_ref2 = this.tools[this.selection]) != null) {
            _ref2.mark.destroy();
          }
          break;
        case TAB:
          if (shiftIsDown) {
            if (this.selection > 0) {
              e.preventDefault();
              this.tools[this.selection - 1].select();
            }
          } else if (this.selection !== this.tools.length - 1) {
            e.preventDefault();
            this.tools[this.selection + 1].select();
          }
      }
    ...
    MarkingSurface.prototype.onBlur = function() {
      var _ref2;
      return (_ref2 = this.tools[this.selection]) != null ? _ref2.deselect() : void 0;
    };

    MarkingSurface.prototype.createTool = function() {
      var tool,
        _this = this;
      tool = new this.tool({
        surface: this
      ...
      tool.on('select', function() {
        var _ref2;
        _this.el.focus();
        if (_this.tools[_this.selection] === tool) {
          return;
        }
        if ((_ref2 = _this.tools[_this.selection]) != null) {
          _ref2.deselect();
        }
        _this.selection = _this.tools.indexOf(tool);
        return _this.trigger('select-tool', [tool]);
      });
      tool.on('deselect', function() {
        _this.selection = -1;
        return _this.trigger('deselect-tool', [tool]);
      });
      ...
      this.trigger('create-mark', [tool.mark]);
      this.triggerEvent('create-mark', tool.mark);
